...
import { Calendar, CircleDollarSign, Hash, ScanLine, ShieldEllipsis, Users } from "lucide-react";
...
const Payments = () => {
    ...
    const { data, isLoading, isError } = useQuery({
        ...
        queryFn: async () => {
            const res = await api.get("/core/overview/payments/");
            return res.data;
        },
        staleTime: 300_000,
    });
    ...
    return (
        <div className='space-y-4'>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" >
                <AnalyticCard
                    title="Total Revenue"
                    value={isLoading ? "..." : `$${data?.total_revenue?.toLocaleString() ?? "0"}`}
                    icon={<CircleDollarSign size={25} />}
                />
                <AnalyticCard
                    title="Total Transactions"
                    value={isLoading ? "..." : `${data?.total_transactions ?? "0"}`}
                    icon={<ScanLine size={25} />}
                />
                <AnalyticCard
                    title="Succeeded Payments"
                    value={isLoading ? "..." : `${data?.total_succeeded ?? "0"}`}
                    icon={<Calendar size={25} />}
                />
                <AnalyticCard
                    title="Pending Payments"
                    value={isLoading ? "..." : `${data?.total_pending ?? "0"}`}
                    icon={<ShieldEllipsis size={25} />}
                />
                <AnalyticCard
                    title="Failed Total Amount"
                    value={isLoading ? "..." : `$${data?.failed_total_amount?.toLocaleString() ?? "0"}`}
                    icon={<Users size={25} />}
                />
            </div>
                ...
                <div className="col-span-3 space-y-4">
                    <div className="rounded-xl bg-[#F8F9FA] p-2 h-full flex flex-col">
                        <div className="flex items-center justify-between px-4 pt-2 pb-4">
                            <h5 className="text-lg font-medium">All Scans</h5>
                            <h5 className="text-lg font-medium">$62,302</h5>
                        </div>
                        <div className='bg-white rounded-xl shadow-xl shadow-[#0A0D1408] border border-[#E9ECEF] p-4 flex-1'>

                            <div className="mb-6">
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-[18px] font-medium text-[#222]">Cost per Project</span>
                            ...
                            <div className="mb-6">
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-[18px] font-medium text-[#222]">30% Deposits Received</span>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-[18px] font-medium text-[#222]">Scan Payments Done</span>

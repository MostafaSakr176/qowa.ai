...
const Scans = ({ params }: { params: { id: string } }) => {
    const [isModalOpen, setIsModalOpen] = useState(false);
    const { data: session } = useSession();

    // TanStack Query for organization details
    const { data: organization, refetch, isLoading } = useQuery({
        queryKey: ["organization-details", params.id, session?.accessToken],
        queryFn: async () => {
            const res = await api.get(
                `/client/organizations/${params.id}/`);
            return res.data;
        },
    ...
    return (
        <div>
            ...
            <div className='rounded-xl bg-[#F8F9FA] p-2'>
                <ScansList organizationId={params.id} />
            </div>
        </div>
    )
}
